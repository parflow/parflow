...
void         TempSource(temp_source, problem, problem_data, time)
Vector      *temp_source;
Problem     *problem;
...
double       time;
{
   ...
   Grid             *grid = VectorGrid(temp_source);
   ...
   case 0:
   {
	 ...
	 ForSubgridI(is, subgrids)
	 {
            ...
            ps_sub  = VectorSubvector(temp_source, is);
...
/*--------------------------------------------------------------------------
 * TempSourceInitInstanceXtra
 *--------------------------------------------------------------------------*/

PFModule  *TempSourceInitInstanceXtra(grid)
Grid    *grid;
{
   PFModule      *this_module   = ThisPFModule;
   PublicXtra    *public_xtra   = PFModulePublicXtra(this_module);
...
/*--------------------------------------------------------------------------
 * TempSourceFreeInstanceXtra
 *--------------------------------------------------------------------------*/

void  TempSourceFreeInstanceXtra()
{
   PFModule      *this_module   = ThisPFModule;
   PublicXtra    *public_xtra   = PFModulePublicXtra(this_module);
...
/*--------------------------------------------------------------------------
 * TempSourceNewPublicXtra
 *--------------------------------------------------------------------------*/

PFModule  *TempSourceNewPublicXtra()
{
   PFModule      *this_module   = ThisPFModule;
   ...
   public_xtra = ctalloc(PublicXtra, 1);
      ...
      switch_name = GetString("TempSources.Type");
	 ...
	 case 0:
	 {
	    ...
	    switch_name = GetString("TempSources.GeomNames");
	    ...
	    for (ir = 0; ir < num_regions; ir++)
	    {
		  ...
		  NA_NameToIndex(GlobalsGeomNames,
	       ...
	       sprintf(key, "TempSources.Geom.%s.Value", NA_IndexToName(dummy0 -> regions, ir));
	 ...
	 case 1:
	 {
	    ...
	    switch_name = GetString("TempSources.PredefinedFunction");
	 ...
	 case 2:
		 {
			 ...
			 dummy2 -> filename = GetString("TempSources.FileName");
...
/*-------------------------------------------------------------------------
 * TempSourceFreePublicXtra
 *-------------------------------------------------------------------------*/

void  TempSourceFreePublicXtra()
{
   PFModule    *this_module   = ThisPFModule;
   PublicXtra  *public_xtra   = PFModulePublicXtra(this_module);
...
/*--------------------------------------------------------------------------
 * TempSourceSizeOfTempData
 *--------------------------------------------------------------------------*/

int  TempSourceSizeOfTempData()
{
   return 0;
}
