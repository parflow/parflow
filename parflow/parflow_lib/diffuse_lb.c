...
*/
      ...
      n_iter++;
      if(!(n_iter%100)) amps_Printf("   ... iter=%d; t=%e; dump=%e; stop=%e\n", 
      n_iter, (lattice->t), write_pressure_time, (lattice->stop) );
  ...
  Problem  *problem;
{
   ...
   {
      c[0] = 1.0/3.0;
      for(i=1; i<=6; i++) c[i] = 1.0/9.0;
   }

   else if(nDirections == 19)
   {
      c[0] = 1.0/3.0;
      for(i=1; i<nDirections; i++) c[i] = 1.0/18.0;
   }
   ...
   L2min = ds[0]*ds[0];
   if(ds[1]*ds[1] < L2min) L2min = ds[1]*ds[1];
   if(ds[2]*ds[2] < L2min) L2min = ds[2]*ds[2];

   L2max = ds[0]*ds[0];
   if(ds[1]*ds[1] > L2max) L2max = ds[1]*ds[1];
   if(ds[2]*ds[2] > L2max) L2max = ds[2]*ds[2];
   ...
   if (nDirections == 7) (lattice->step) /= 10;
   else if (nDirections == 19) (lattice->step) /= 10;
...
Vector *field;
{
   ...
   ForSubgridI(i_s, GridSubgrids(grid))
   {
      ...
      {
         tmp = fabs( fp[fi] );
         if ( tmp > max_vector_value ) max_vector_value = tmp;
      });
   }
...
Vector *field1, *field2;
{
   ...
   ForSubgridI(i_s, GridSubgrids(grid))
   {
      ...
      {
         if (f2p[fi] == 0.0) tmp = 0.0;
         else                tmp = fabs( f1p[fi]/f2p[fi] );
