...
Statistics   *stats;
{
   ...
   *-----------------*/
   ...
   for (gridloop = 0; gridloop < GridNumSubgrids(grid); gridloop++)
   {
      ...
      if(iLy > iLx) max_search_radius = iLy;
      if(iLz > iLy) max_search_radius = iLz;
      ...
      if (nc_sub)
      {
	    ...
	    index1 = SubvectorEltIndex(sub_field, i, j, k);
	       ...
	       cmean = 0.0;
	       csigma = 0.0;
               for (n=0; n<cpts; n++) w[n] = b[n];
             
               dpofa_(A, &cpts, &cpts, &ierr);
               dposl_(A, &cpts, &cpts, w);

               for(m=0; m<cpts; m++) cmean += w[m]*value[m]; 

               for (m=0; m<cpts; m++) csigma += w[m]*b[m]; 
