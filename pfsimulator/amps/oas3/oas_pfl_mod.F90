module oas_pfl_mod
  ...
  integer :: nlevsoil = 20          ! Number of hydrologically active soil layers in eCLM
  integer :: nlevgrnd = 25          ! nlevsoil + 5 bedrock layers
...
contains
  ...
  subroutine oas_pfl_define(nx, ny, pdx, pdy, ix, iy, sw_lon, sw_lat, nlon, nlat, &
    ...
    ! -----------------------------------------------------------------
    call oasis_def_var (et_id, "PFL_ET", part_id, (/1, nlevsoil/), OASIS_In, OASIS_Real, ierror)
    if (ierror /= 0) call oasis_abort(comp_id, 'oas_pfl_define', 'oasis_def_var failed for PFL_ET')

    call oasis_def_var (sat_id, "PFL_SAT", part_id, (/1, nlevgrnd/), OASIS_Out, OASIS_Real, ierror)
    if (ierror /= 0) call oasis_abort(comp_id, 'oas_pfl_define', 'oasis_def_var failed for PFL_SAT')

    call oasis_def_var (psi_id, "PFL_PSI", part_id, (/1, nlevgrnd/), OASIS_Out, OASIS_Real, ierror)
  ...
  subroutine send_fld2_clm(pressure, saturation, topo, ix, iy, nx, ny, nz, nx_f, ny_f, pstep) bind(c, name='send_fld2_clm_')
    ! '''
    ! Based from ../pfsimulator/amps/oas3/send_fld2_clm.F90
    ! 
    ! Description:
    !   Sends pressure head, soil saturation and porosity from ParFlow to CLM
    ...
    !allocate(sat_snd(nx,ny,nlevgrnd))
    allocate(sat_snd(nx,ny,nlevgrnd))
  ...
  subroutine receive_fld2_clm(evap_trans, topo, ix, iy, nx, ny, nz, nx_f, ny_f, pstep) bind(c, name='receive_fld2_clm_')
    ! '''
    ! Based from ../pfsimulator/amps/oas3/receive_fld2_clm.F90
    ! 
    ! Description:
    !   This routine receives evapotranspiration flux from OASIS3 coupler via eCLM
    ! '''
