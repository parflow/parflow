                 ...
                 int *size)
{
  ...
  MPI_Probe(src, 0, oas3Comm, &status);

  MPI_Get_count(&status, MPI_BYTE, size);

  buf = (char*)malloc((size_t)(*size));

  MPI_Recv(buf, *size, MPI_BYTE, src, 0, oas3Comm, &status);
...
int amps_Recv(amps_Comm comm, int source, amps_Invoice invoice)
{
  ...
  MPI_Probe(source, 0, oas3Comm, &status);

  MPI_Get_count(&status, MPI_BYTE, &size);

  buffer = (char*)malloc((size_t)(size));

  MPI_Recv(buffer, size, MPI_BYTE, source, 0, oas3Comm, &status);
