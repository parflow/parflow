 ...
 */
#define amps_TAlloc_managed(type, count) \
        ((count > 0) ? (type*)_amps_talloc_device((size_t)(count) * sizeof(type)) : NULL)

/**
 * Same as \ref amps_CTAlloc but allocates managed memory
 */
#define amps_CTAlloc_managed(type, count) \
        ((count) ? (type*)_amps_ctalloc_device((size_t)(count) * sizeof(type)) : NULL)
