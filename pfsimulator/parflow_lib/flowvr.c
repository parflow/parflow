...
void vectorToMessage(Vector* v, fca_message *result) {
  ...
  *result = fca_new_message(moduleParflow, sizeof(double)*nx*ny*nz);
  // write to the beginning of our memory segment
  double* buffer = (double*) fca_get_write_access(&result, 0);
  double *data;
  data = SubvectorElt(subvector, ix, iy, iz);

  // some iterators
  int i, j, k, d = 0, ai = 0;
  BoxLoopI1(i, j, k, ix, iy, iz, nx, ny, nz, ai, nx_v, ny_v, nz_v, 1, 1, 1,{ buffer[d] = data[ai]; d++;});
...
#define n_portnamedata (sizeof (portnamedatas) / sizeof (const PortNameData))
  ...
  for (unsigned int i = 0; i < n_portnamedata; ++i)
  {
    ...
    // Prepare the Message
    fca_message msg;
    vectorToMessage(portnamedatas[i].data, &msg);
