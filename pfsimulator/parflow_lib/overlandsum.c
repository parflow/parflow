        ...
        case 7:
        {
          ForSubgridI(is, GridSubgrids(grid))
          {
            ...
            for (i = ix; i < ix + nx; i++)
            {
			  ...
			  if(overland_sum_ptr[index_overland_sum] != 0.0)
			  {
			    printf("i=%d j=%d k=%d overland_sum=%f\n",i,j,k,overland_sum_ptr[index_overland_sum]);
			  }
                        }
                      }
                    }
  ...
  ForSubgridI(is, GridSubgrids(grid))
  {
	...
	switch (BCStructBCType(bc_struct, ipatch))
	{
	  case OverlandBC:
	  {
		...
		overlandflow_cell_outflow_dat[io] += ((pfmax(kn_dat[io], 0) + pfmax(-ks_dat[io], 0)) * dx + (pfmax(-ke_dat[io], 0) + pfmax(kw_dat[io], 0)) * dy) * dt ;

		overlandflow_face_flow_dat[OverlandFlowKE][io] += ke_dat[io] * dy * dt;
		overlandflow_face_flow_dat[OverlandFlowKW][io] += kw_dat[io] * dy * dt;
		overlandflow_face_flow_dat[OverlandFlowKN][io] += kn_dat[io] * dx * dt;
		overlandflow_face_flow_dat[OverlandFlowKS][io] += ks_dat[io] * dx * dt;

		if(overlandflow_cell_outflow_dat[io] != 0.0)
		{
		  printf("i=%d j=%d k=%d outflow=%f ke_dat=%f kw_dat=%f kn_dat=%f ks_dat=%f\n",i,j,k,overlandflow_cell_outflow_dat[io],ke_dat[io],kw_dat[io],kn_dat[io],ks_dat[io]);
		}
