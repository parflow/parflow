...
!  print*, clm_bin_out_dir
  ...
  if (clm_bin_out_dir == 0) then
     ...
     open(1997,file=clm_output_dir//'qflx_evap_grnd/qflx_evap_grnd.'//cistep//'.bin.'//trim(adjustl(RI)), access=ACCESS, form=FORM)  ! @ 2D output file     
     write(1997)ix,iy,iz,drv%nc,drv%nr,nz
     ...
     open(2002,file=clm_output_dir//'qflx_evap_tot/qflx_evap_tot.'//cistep//'.bin.'//trim(adjustl(RI)), access=ACCESS, form=FORM)   ! @ 2D output file     
     write(2002)ix,iy,iz,drv%nc,drv%nr,nz
     ...
     open(2006,file=clm_output_dir//'eflx_lwrad_out/eflx_lwrad_out.'//cistep//'.bin.'//trim(adjustl(RI)), access=ACCESS, form=FORM) ! @ 2D output file 
     write(2006)ix,iy,iz,drv%nc,drv%nr,nz
