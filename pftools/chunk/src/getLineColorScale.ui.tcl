...
proc getLineColorScale_ui {root i args} {
...
#        
        ...
        entry $base.entryRangeMin -textvariable line_value_min($i) -width 10 \
           -background #bdd -highlightcolor #f00
        entry $base.entryRangeMax -textvariable line_value_max($i) -width 10 \
           -background #bdd -highlightcolor #f00
        entry $base.entryScaleFactor -textvariable line_time_scale($i) \
        ...
        button $base.colorTable -text "Color Table:" -background #e8e \
             -borderwidth 4 \
             -command "OpenLineColorTableFile $base $i" -font {times 12 bold}  
        label $base.labelColorTable -textvariable line_color_table($i)
        grid $base.colorTable -in $base.top -row 1 -column 0
        grid $base.labelColorTable -in $base.top -row 1 -column 1 \
             -columnspan 6 -sticky w 

        proc OpenLineColorTableFile {root i args} {
                      global line_color_table
                      set typelist {
                          {"CTB Files" {".ctb"} {"CTB "}}
                      }
                      set color_table_dir [file dirname $line_color_table($i)]
                      set line_color_table_temp \
                      [tk_getOpenFile -defaultextension .ctb \
                      -filetypes $typelist \
                      -initialdir $color_table_dir \
                      -initialfile [file tail $line_color_table($i)] \
                      -parent $root \
                      -title {"Open color table file for line $i"}]
                      if {$line_color_table_temp != ""} {
                        set line_color_table($i) $line_color_table_temp
                      }
                    }
...
# 
        ...
        entry $base.entrySpacing -textvariable line_legend_label_spacing($i) \
           -width 10 -background #bdd -highlightcolor #f00
       label $base.decimalPlaces -text "  .Decimal Places" -font {times 12 bold}
        entry $base.entryDecimal -textvariable line_legend_label_decimals($i) \
           -width 1 -background #bdd -highlightcolor #f00
        radiobutton $base.radioDec0 -text 0 -value 0 \
            -variable line_legend_label_decimals($i)
        ...
        entry $base.entryTicsPerLabel \
            -textvariable line_legend_label_tics_per($i) \
            -width 1 -background #bdd -highlightcolor #f00
        radiobutton $base.ticsPer1 -text 1 -value 1 \
            -variable line_legend_label_tics_per($i) 
        ...
        scale $base.scaleTransX -from 0 -to 500 -length 250 \
            -variable line_legend_translation_x($i) -orient horizontal \
            -tickinterval 100 -showvalue true
        scale $base.scaleTransY -from 0 -to 500 -length 250 \
            -variable line_legend_translation_y($i) -orient horizontal \
            -tickinterval 100 -showvalue true
