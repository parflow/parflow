proc getDots_ui {root args }  {
	...
	frame $base.top$i 
	grid $base.top$i -in $base.dots -row $i -column 0
	button $base.getDotsFile$i -background #e8e -borderwidth 4 \
	    -text "File $i" -font {times 12 bold} \
	    -command "OpenDotsFile $base $i"
	grid $base.getDotsFile$i -in $base.top$i -row 0 -column 0 \
	    -sticky e
	entry $base.dotsFileName$i -textvariable dots_file($i) \
	    -width 80 -background #bdd -highlightcolor #f00
	grid $base.dotsFileName$i -in $base.top$i -row 0 -column 1
	frame $base.dotsFormats$i
	grid $base.dotsFormats$i -in $base.top$i -row 1 -column 0 \
	    -sticky nw
	label $base.labelFormat$i -text Format: -font {times 12 bold}
	grid $base.labelFormat$i -in $base.dotsFormats$i \
	    -row 0 -column 0 -sticky w
	set j 1
	foreach x {name} {
	    if {$j==1} {set lab "name & \n location"}
	    if {$j==2} {set lab "# of \n points"}
	    if {$j==3} {set lab "weight \n change"}
	    if {$j==4} {set lab "geoeas \n indic."}
	    radiobutton $base.$i$x -text $lab -value $x \
		-variable dots_format($i)
	    grid $base.$i$x -in $base.dotsFormats$i -row $j -column 0 \
		-sticky nw 
	    set j [expr $j + 1]
	}
	...
	label $base.labelAppearance$i -text "DOTS $i APPEARANCE" \
	    -font {times 12 bold}
	...
	grid $base.marker$i -in $base.appearance$i -row 0 -column 5 
	scale $base.outsideWidth$i -from 0.0 -to 10 -length 72 \
	    -variable dots_outside_size($i)  -orient horizontal \
	    -tickinterval 10 -showvalue true -resolution 0.5 -digits 3 \
	    -sliderlength 18 
	scale $base.insideWidth$i -from 0.0 -to 10.0 -length 72 \
	    -variable dots_inside_size($i)  -orient horizontal \
	    -tickinterval 10.0 -showvalue true -resolution 0.5 -digits 3 \
	    -sliderlength 18 
	scale $base.outsideMinWidth$i -from 0.0 -to 10.0 -length 72 \
	    -variable dots_outside_size_min($i)  -orient horizontal \
	    -tickinterval 10.0 -showvalue true -resolution 0.5 -digits 3 \
	    -sliderlength 18 
	scale $base.insideMinWidth$i -from 0.0 -to 10.0 -length 72 \
	    -variable dots_inside_size_min($i)  -orient horizontal \
	    -tickinterval 0.0 -showvalue true -resolution 0.5 -digits 3 \
	    -sliderlength 18
	...
	#
        ...
        grid $base.frameInsideMarker$i -in $base.appearance$i -row 2 -column 5 

        set j 0
        foreach marker {o + x } {
	    set j [expr $j + 1]
	    radiobutton $base.pickMarkerIn1$i$j -text $marker \
		-variable dots_inside_marker($i) \
		-value $j
	...
	#       grid $base.checkInsideMarker$i -in $base.frameInsideMarker$i \
        ...
        grid $base.frameOutsideMarker$i -in $base.appearance$i -row 1 -column 5 

        set j 0
        foreach marker {o + x } {
	    set j [expr $j + 1]
	    radiobutton $base.pickMarkerOut1$i$j -text $marker \
		-variable dots_outside_marker($i) \
		-value $j
	...
	#        checkbutton $base.checkOutsideMarker$i \
	    #           -variable dots_outside_sign($i) \
	    #           -text only -onvalue -1 -offvalue 1

        radiobutton $base.radioOutsideMarker$i -text "none" \
	    -variable dots_outside_marker($i) -value 0   
	...
	#        grid $base.checkOutsideMarker$i -in $base.frameOutsideMarker$i \
        ...
        entry $base.entryX$i -textvariable dots_origin_x($i) \
	    -width 12 -background #bdd -highlightcolor #f00
        entry $base.entryY$i -textvariable dots_origin_y($i) \
	    -width 12 -background #bdd -highlightcolor #f00
        entry $base.entryZ$i -textvariable dots_origin_z($i) \
	    -width 12 -background #bdd -highlightcolor #f00
	...
	button $base.scalecolor$i -text "Color Scale" -background "#e8e" \
	    ...
	    -command "toplevel .getDotsColorScale; \
                      focus .getDotsColorScale; \
                      corner .get_Dots .getDotsColorScale; \
                      wm title .getDotsColorScale {Color Scale for Dot $i}; \
                      getDotsColorScale_ui .getDotsColorScale $i; \
                      grab .getDotsColorScale; \
                      tkwait window .getDotsColorScale"

	label $base.winnow$i -text "Winnow" -font {times 12 bold}
	entry $base.entryWinnow$i -textvariable dots_winnow($i) \
...
proc getMarker_ui {root m args} {
    global dots_outside_marker tcl_dir
    # this treats "." as a special case
    if {$root == "."} {
	set base ""
    } else {
	...
	set dots_outside_marker($m) $marker 
	button $base.buttonOK \
	    -text "OK" \
	    -font {times 12 bold} \
	    -background "#9f9" \
...
proc OpenDotsFile {w k } {
    global dots_file
    global home_dir
    if {[file exists $dots_file($k)] == 1} {
	set dots_dir [file dirname $dots_file($k)]
    } else {
	set dots_dir $home_dir
    }
    set dots_file_temp [tk_getOpenFile \
			    -initialdir $dots_dir \
			    -initialfile [file tail $dots_file($k)] \
			    -parent $w \
			    -title "Open Dot File $k"]
    if {$dots_file_temp != ""} {
	set dots_file($k) $dots_file_temp
    }
}   
