...
%Further variables may be added in future.
...
\begin{enumerate}
\item create the \parflow{} input database and prepare the input files (\code{tclsh problem.tcl})
...
\item the \code{<problemname>.tcl} file produces an input database named
  \code{<problemname>.pfidb} with the \code{pfwritedb} command
...
The \code{parflowvr.py} contains:
\begin{display}\begin{verbatim}
from filters import *
import os, sys
