...
directory \file{\$PARFLOW_SRC/docs}.
Various style files used to build the \parflow{} manuals are located
in the directory \file{\$PARFLOW_SRC/docs/lib}.
Files ending in \code{.sty} are used in LaTeX to create printed
versions of the manuals, and files ending in \code{.perl} (these files
are perl scripts) are used by LaTeX2HTML to create online versions
of the manuals.
The files \file{html.sty} and \file{html.perl}
...
The files \file{parflow.sty} and \file{parflow.perl}
...
be advisable to invoke the \file{Makefile} directly to build
...
\kbd{build veryclean} between builds; there are times when 
...
into the appropriate subdirectories of \file{\$PARFLOW_HELP}.
The install also changes the group and permissions on the files so
that others may read them using \code{pfhelp}.
