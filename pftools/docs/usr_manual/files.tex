...
Richards \cite{JW01} unless you’re running a totally saturated domain (solver 
IMPES).
...
{How many times to reuse a CLM input. For example timestep=1, 
reuse =1 is normal behavior but reuse=2 and timestep=0.5 subdivides 
the time step using the same CLM input for both halves instead of needing two files.} 
\begin{display}\begin{verbatim}
