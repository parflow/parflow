...
Chunking may have significant impact in writing output. If this key is not set, default chunking scheme will be used by NetCDF library. Chunks are hypercube(hyperslab) of any dimension. When chunking is used, chunks are written in single write operation which can reduce access times. For more information on chunking, refer to NetCDF4 user guide.
...
\textit{\textbf{Node level collective I/O is currently implemented for 2-D domain decomposition only.}}
