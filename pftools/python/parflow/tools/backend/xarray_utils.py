...
def add_decade(ds: xr.Dataset, time_name="time") -> xr.Dataset:
    ...
    dt = ds[time_name].dt
    decade = pd.to_datetime({'year': dt.year, 'month': dt.month, 'day': (dt.day // 10).clip(max=2) + 1})
    return ds.assign_coords(decade=(time_name, decade))


def xr_pf_wtd(press: xr.DataArray, satur: xr.DataArray, ssat: Optional[Number | xr.DataArray] = None,
              epsilon: float = 0.001) -> xr.DataArray:
    ...
    def wtd_fct(press, satur, dz, ssat, epsilon):
        ...
        res = calculate_water_table_depth(press.data.squeeze(), satur.data.squeeze(), dz, ssat, epsilon)
    ...
    res = xr.map_blocks(wtd_fct, press, (satur,), kwargs={"dz": dz, "ssat": ssat, "epsilon": epsilon},
                        template=press.isel(z=0).drop_vars('z'))

    return res.rename("wtd").assign_attrs(dict(units="m"))


def xr_pf_flow(press: xr.DataArray, slopex: xr.DataArray, slopey: xr.DataArray, mannings: xr.DataArray) -> xr.DataArray:
    ...
    def flow_fct(press, slopex, slopey, mannings, dx, dy):
        ...
        res = calculate_overland_flow_grid(press_data, slopex.data.squeeze(), slopey.data.squeeze(), mannings=mannings.data.squeeze(), dx=dx, dy=dy, mask=mask)
        res = np.expand_dims(res, axis=0) # from x, y to t, x, y
    ...
    res = xr.map_blocks(flow_fct, press, (slopex, slopey, mannings), kwargs={"dx": dx, "dy": dy}, template=press.isel(z=0).drop_vars('z'))
...
def xr_pf_evaptrans(evaptrans: xr.DataArray) -> xr.DataArray:
    ...
    res = xr.map_blocks(evap_fct, evaptrans, kwargs={"dx": dx, "dy": dy, "dz": dz}, template=evaptrans)

    return res.rename("evaptrans").assign_attrs(dict(units="L^3/T"))


def xr_pf_subsurface_storage(press: xr.DataArray, satur: xr.DataArray,
                             specific_storage: xr.DataArray, porosity: xr.DataArray) -> xr.DataArray:
    ...
    def subsurface_storage_fct(press, satur, specific_storage, porosity, dx, dy, dz):
        # squeeze from t, z, x, y to z, x, y
        res = calculate_subsurface_storage(porosity.data.squeeze(), press.data.squeeze(),
                                           satur.data.squeeze(), specific_storage.data.squeeze(),
                                           dx=dx, dy=dy, dz=dz)
        res = np.expand_dims(res, axis=0) # from x, y to t, x, y
    ...
    res = xr.map_blocks(subsurface_storage_fct, press, (satur, specific_storage, porosity),
                         kwargs={"dx": dx, "dy": dy, "dz": dz}, template=press)
...
def xr_pf_surface_storage(press: xr.DataArray) -> xr.DataArray:
    ...
    res = xr.map_blocks(surface_storage_fct, press,
                         kwargs={"dx": dx, "dy": dy}, template=press.isel(z=0).drop_vars('z'))

    return res.rename("surface_storage").assign_attrs(dict(units="m^3"))


def xr_pf_trend(var: xr.DataArray, var_mean: Optional[xr.DataArray] = None) -> xr.DataArray:
    ...
    a = ((t_num[:, np.newaxis, np.newaxis] - t_mean) * (var - var_mean)).mean("time") / ((t_num - t_mean) ** 2).mean()
...
def xr_pf_regression(var: xr.DataArray, var_mean: Optional[xr.DataArray] = None) -> xr.DataArray:
    ...
    a = ((t_num[:, np.newaxis, np.newaxis] - t_mean) * (var - var_mean)).mean("time") / ((t_num - t_mean) ** 2).mean()
    b = var_mean - a * t_mean
    trend = np.expand_dims(a, axis=0) * t_num[:, np.newaxis, np.newaxis] + np.expand_dims(b, axis=0)
...
def xr_pf_recharge(wtd: xr.DataArray, porosity: Optional[xr.DataArray], effective: bool = False, emptying: bool = False):
    ...
    if effective:
        if porosity is None:
            print("Unable to calculate effective recharge without porosity (porosity=None), skipping.")
        else:
            """ Idea :
            wtd       8
            z         [100 30  10  5   3   1   1  ]
            ...
            porosity_data = np.expand_dims(porosity.data, axis=1) # len(z), x, y -> len(z), 1, x, y
    ...
    if emptying:
        ...
        emptying_mean = emptying_da.sum(axis=0) / np.maximum(np.count_nonzero(emptying_da, axis=0), 1)
