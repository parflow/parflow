...
from parflow.tools.backend.xarray_utils import (
    xr_pf_wtd,
    xr_pf_evaptrans,
    xr_pf_flow,
    xr_pf_surface_storage,
    xr_pf_subsurface_storage,
    xr_pf_trend,
    xr_pf_recharge,
    add_decade,
    xr_pf_regression,
...
# Reading parameters
kwargs = dict(start_date="2015-01-01", time_label="left", default_forcing_ts=10800)
...
def export_press():
    ds = xr.open_dataset(
        simu_path, engine="pf_ige", select_variables="press", **kwargs
    ).astype("float32")
...
def export_prcp_decade():
    ...
    ds.to_netcdf(
        "res/prcp_decade_latlon.nc", encoding=encoding, unlimited_dims=unlimited_dims
    )
...
def export_instant_evap():
    ...
    ds.to_netcdf(
        "res/instant_evap_daily.nc", encoding=encoding, unlimited_dims=unlimited_dims
    )
...
def export_wtd():
    ds = xr.open_dataset(
        simu_path,
        engine="pf_ige",
        select_variables=["press", "satur", "porosity"],
        **kwargs
    ).astype("float32")
    ...
    ds_final["quality"] = np.abs(
        ds_final["amplitude"] - xr_pf_trend(ds_final["wtd"], ds_final["wtd_mean"])
    ) / np.where(ds_final["amplitude"] > 0, ds_final["amplitude"], np.nan).astype(
        "float32"
    )

    ds_final["recharge"] = xr_pf_recharge(
        ds_final["wtd"], ds["porosity"], effective=True
    )

    ds_final["recharge_emptying"] = xr_pf_recharge(
        ds_final["wtd"], ds["porosity"], effective=True, emptying=True
    )
...
def export_params():
    ...
    ds = xr.open_dataset(
        simu_path, engine="pf_ige", select_types="input", **kwargs
    ).astype("float32")
...
def export_evaptrans():
    ds = xr.open_dataset(
        simu_path, engine="pf_ige", select_variables="evaptrans", **kwargs
    ).astype("float32")
...
def export_storage():
    ds = xr.open_dataset(
        simu_path, engine="pf_ige", select_types=["output", "temporal_output"], **kwargs
    ).astype("float32")

    subsurface_storage = xr_pf_subsurface_storage(
        ds["press"], ds["satur"], ds["specific_storage"], ds["porosity"]
    )

    subsurface_storage.to_netcdf(
        "res/subsurface_storage.nc", encoding=encoding, unlimited_dims=unlimited_dims
    )

    surface_storage = xr_pf_surface_storage(ds["press"])

    surface_storage.to_netcdf(
        "res/surface_storage.nc", encoding=encoding, unlimited_dims=unlimited_dims
    )
...
def export_flow():
    ds = xr.open_dataset(
        simu_path,
        engine="pf_ige",
        select_variables=["press", "slopex", "slopey", "mannings"],
        **kwargs
    ).astype("float32")

    flow = xr_pf_flow(
        ds["press"], ds["input_slopex"], ds["input_slopey"], ds["mannings"]
    )
