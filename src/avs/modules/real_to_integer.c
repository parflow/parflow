...
realToInt()
{
  int realToIntCompute();	/* declare the compute function (below) */
  int in_port, out_port;	/* temporaries to hold the port numbers */
  
  /* Set the module name and type */
  AVSset_module_name("real to integer", MODULE_FILTER);
  ...
  out_port = AVScreate_output_port("Output Integer",
				   "integer");
  
  AVSautofree_output(out_port);

  /* Tell avs what subroutine to call to do the compute */
  AVSset_compute_proc(realToIntCompute);
  AVSset_module_flags(COOPERATIVE | REENTRANT);
}


realToIntCompute(input, output, rounding)
float *input;
int *output;
char *rounding;
{
