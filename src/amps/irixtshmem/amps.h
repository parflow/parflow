...
#define amps_Rank(comm) (int)(PRDA -> usr_prda.fill[0])
#define amps_rank ((int)(PRDA -> usr_prda.fill[0]))

#define AMPS_MAX_THREADS 20
#define amps_ThreadLocal(arg) arg[amps_rank]
#define amps_ThreadLocalDcl(type, arg) type arg[AMPS_MAX_THREADS]

extern int amps_size;

extern usptr_t *amps_arena;
...
#define amps_SyncInvoice (amps_PtrSyncInvoice[amps_Rank(amps_CommWorld)])
...
#define amps_new(comm, size) ((char *)usmalloc(size+sizeof(double), amps_arena) + sizeof(double))
#define amps_free(comm, buf) usfree((char*)buf - sizeof(double), amps_arena)
...
#ifdef NO_SHMEM_SYNC
#define amps_Sync(comm) amps_AllReduce((comm), amps_SyncInvoice, amps_SyncOp)
#else
#define amps_Sync(comm) barrier(amps_shmem_info -> sync, amps_size)
#endif
...
#define amps_TFree(ptr) if (ptr) usfree(ptr, amps_arena); else
...
#define amps_TAlloc(type, count) \
((count) ? (type *) usmalloc((unsigned int)(sizeof(type) * (count)), amps_arena): NULL)
 
#define amps_CTAlloc(type, count) \
((count) ? (type *) uscalloc((unsigned int)(count), (unsigned int)sizeof(type), amps_arena) : NULL)
...
/* The maximum number of shared memory processors allowed */
/* Make static since it is not very large */
#define AMPS_MAX_NODES 256
...
typedef struct amps_shmeminfo
{
   /* barrier for doing syncs */
   barrier_t *sync;

   barrier_t *bcast_lock;

   /* locks for each nodes message queue */
   ulock_t locks[AMPS_MAX_NODES];
   usema_t *sema[AMPS_MAX_NODES];

   AMPS_ShMemBuffer *buffers[AMPS_MAX_NODES];
   AMPS_ShMemBuffer *buffers_end[AMPS_MAX_NODES];

   AMPS_ShMemBuffer *buffers_local[AMPS_MAX_NODES];
   AMPS_ShMemBuffer *buffers_local_end[AMPS_MAX_NODES];

   ulock_t bcast_locks[AMPS_MAX_NODES];
   usema_t *bcast_sema[AMPS_MAX_NODES];
   AMPS_ShMemBuffer *bcast_buffers[AMPS_MAX_NODES];
   AMPS_ShMemBuffer *bcast_buffers_end[AMPS_MAX_NODES];

   AMPS_ShMemBuffer *bcast_buffers_local[AMPS_MAX_NODES];
   AMPS_ShMemBuffer *bcast_buffers_local_end[AMPS_MAX_NODES];

   ulock_t bcast_mem[AMPS_MAX_NODES];
} AMPS_ShMemInfo;

extern AMPS_ShMemInfo *amps_shmem_info;
