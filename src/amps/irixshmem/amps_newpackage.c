			     ...
			     amps_Invoice *recv_invoices)
{
   ...
   if(num_send)
   {
      package -> snd_info = (amps_SrcInfo **)malloc(sizeof(amps_SrcInfo *)
						     *num_send); 
	 ...
	 if( (src_info -> send_sema = usnewsema (amps_arena, 0)) == NULL)
	    printf("error allocating sema send\n");
	 
	 if( (src_info -> recv_sema = usnewsema (amps_arena, 0)) == NULL)
	    printf("error allocating sema send\n");
   ...
   if(num_recv)
   {
      package -> rcv_info = (amps_SrcInfo **)malloc(sizeof(amps_SrcInfo *)
						    *num_recv);
...
void amps_FreePackage(amps_Package package)
{
   ...
   if(package)
   {
	 ...
	 while(i--)
	 {
	    ...
	    usfreesema(package -> rcv_info[i] -> send_sema, amps_arena);
	    usfreesema(package -> rcv_info[i] -> recv_sema, amps_arena);
