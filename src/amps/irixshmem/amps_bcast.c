...
/* SGS??????? there was a problem here, was allocating a single buffer
   and sending if off but this has a problem with overlay so allocate
   a buffer for each dest */
	 ...
	 *src_ptr = amps_rank;
	 ...
	 while(not_found)
	 {
	    ...
	    if (buf -> next )
	       ...
	       amps_shmem_info -> bcast_buffers_end[amps_rank] = buf -> prev;
