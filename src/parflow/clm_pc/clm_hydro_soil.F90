...
!=== Local Variables =====================================================
  ...
  real(r8) hk(1:nlevsoi),       & ! hydraulic conductivity (mm h2o/s)
       ...
       evx,                     & ! implicit evaporation for soil temperature equation
...
! For Z.-L. Yang & G.-Y. Niu's modification
  ...
  real(r8) storage,tot_storage !@ available storage in the soil column / total storage in the soil column
...
!@ In order to investigate the effects of spatial scale we turn TOPMODEL approach off
!@ qflx_surf is set to zero and will be evaluated with Parflow (depth of ponding on the surface)                             
!  clm%qflx_surf =  max(0., fcov*clm%qflx_top_soil) + &
!                   max(0., (1.-fcov)*min(1.,wmean**4)*clm%qflx_top_soil)

!(1.0*clm%watsat(1) - clm%pf_vol_liq) * clm%dz(1) * 1000.0d0 - clm%qflx_top_soil * clm%dtime

!@ Calculate available storage for the entire soil column
!storage = 0.0d0
!tot_storage = 0.0d0
!do i = 1, parfl_nlevsoi
! storage = storage + clm%pf_vol_liq(i) * clm%dz(1) * 1000.0d0
! tot_storage = tot_storage + clm%watsat(i)*clm%dz(1) * 1000.0d0
!enddo
!storage = tot_storage - storage 
!storage = storage + clm%dtime*abs(clm%qflx_tran_veg)
 
!if (((clm%qflx_top_soil-abs(clm%qflx_evap_grnd))*clm%dtime) > storage) then
! clm%qflx_infl = storage / clm%dtime - storage / clm%dtime * 1.0d-2 !@ the latter term prevents complete saturation
! clm%qflx_surf = clm%qflx_top_soil - clm%qflx_infl - clm%qflx_evap_grnd + storage / clm%dtime * 1.0d-2
!else 
! clm%qflx_surf = 0.0d0
! clm%qflx_infl = clm%qflx_top_soil - clm%qflx_evap_grnd
!endif
                   
!@ Constant fraction of water becoming surface runoff after the above approach.
!@ This value is used in the sub-timsteps in the overland routing routine to partition 
!@ the water between surface runoff and infiltration.
 clm%frac = max(0.0d0,fcov) + max(0.0d0,(1.0d0 - fcov)*min(1.0d0,wmean**4.0d0))                 
...
!@ This part has been replaced with the mass balance approach above to replace the TOPMODEL approach
!  sdamp = 0.
!  if (clm%snl+1 >= 1) then
