...
subroutine drv_readclmin(drv,grid)
...
!@ Overland routing parameters (read into 2D grid module variables)
     if (vname == 'sdep')            call drv_get2drvar(c,r,grid%sdep)
	 if (vname == 'manning')         call drv_get2drvar(c,r,grid%manning)
	 if (vname == 'W')               call drv_get2drvar(c,r,grid%W)
	 if (vname == 'rout')            call drv_get1divar(drv%rout)
	 if (vname == 'cout')            call drv_get1divar(drv%cout)
	 if (vname == 'sovout')          call drv_get1drvar(drv%sovout)
...
!@ These statements should be placed into "open_files.f90"
  if (drv%startcode == 1)then  !Append to old output file
     open(20,file=drv%outf1d,  form='formatted', position='append')  !Timeseries output
