...
RFCondData   *cdata;
{
   ...
   *-----------------*/
   ...
   if (iLx > max_search_rad) iLx = max_search_rad;
   if (iLy > max_search_rad) iLy = max_search_rad;
   if (iLz > max_search_rad) iLz = max_search_rad;
    ...
    *------------------------*/
     ...
     for(i=0; i<iLxyz; i++) rand_path[i] = talloc(int,3);
     modulus = 2;
     while (modulus < iLxyz+1) modulus *= 2;  
     ...
     {
       r = (5*r + 1) % modulus;
       while ((r<1) || (r>iLxyz)) r = (5*r + 1) % modulus;
    ...
    *-----------------------------------------------------------------------*/
    ...
    for (i=0; i<nLx; i++)
    {
      ...
      for (j=0; j<nLy; j++) cov[i][j] = ctalloc(double, nLz);
    }
    ...
    cy = 0.0;
    cz = 0.0;
    if (lambdaX != 0.0) cx = dx*dx/(lambdaX*lambdaX);
    if (lambdaY != 0.0) cy = dy*dy/(lambdaY*lambdaY);
    if (lambdaZ != 0.0) cz = dz*dz/(lambdaZ*lambdaZ);
    ...
    */
    ...
    for (i= -iLx; i <= 2*iLx; i++)
    {
        ...
        for (j= -iLy; j <= 2*iLy; j++) 
        {
           ...
           marker[i][j] += iLz;
           for (k = -iLz; k <= 2*iLz; k++) marker[i][j][k] = 0;
        }
    }
   ...
   for (gridloop = 0; gridloop < GridNumSubgrids(grid); gridloop++)
   {
      ...
      if ((dist_type == 1) || (dist_type == 3))
      {
         for(n=0; n < nc; n++)
         {
            ...
            {
               index2 = SubvectorEltIndex(sub_tmpRF, i, j, k);
               if (v[n] <= 0.0) tmpRFp[index2] = Tiny;
               else tmpRFp[index2] = (log(v[n]/mean))/sigma;
            }
         }
      }
      ...
      for(n=0; n<iLxyz; n++)
      {
         ...
         if (n > 0) 
         {
            ...
            if (j_search > max_search_rad) max_search_rad = j_search;
            if (k_search > max_search_rad) max_search_rad = k_search;

            /* Reset the comm package based on the new max_search_radius */
            if      (max_search_rad == 1) update_mode = VectorUpdatePGS1;
            else if (max_search_rad == 2) update_mode = VectorUpdatePGS2;
            else if (max_search_rad == 3) update_mode = VectorUpdatePGS3;
            else 			  update_mode = VectorUpdatePGS4;
         ...
         rpy = rand_path[n][1];
         rpz = rand_path[n][2];
  	 ix2 = rpx;  while (ix2 < ix) ix2 += iLxp1;
         iy2 = rpy;  while (iy2 < iy) iy2 += iLyp1;
	 iz2 = rpz;  while (iz2 < iz) iz2 += iLzp1;
		  ...
		  for(i=ix2; i<nxG; i+= iLxp1)
		  {
			...
			if (cpts > 0)
			{
			   ...
			   for (j2=0; j2<npts+cpts; j2++)
			   {
				 ...
				 A = cov[di][dj][dk];
				 if(i2<npts && j2>= npts) A12[i2][j2-npts] = A;
				 if(i2>=npts && j2< npts) A21[i2-npts][j2] = A;
				 if(i2>=npts && j2>= npts) A22[i2-npts][j2-npts] = A;
			      }
			   }

			   /* Compute b2' = b2 - A21 * A11_inv * b1 and augment b1 */
                           for(i2=0; i2<cpts; i2++) b2[i2] = b[i2+npts];
                           for(i2=0; i2<npts; i2++) b_tmp[i2] = b[i2];
                           ...
                           for(i2=0; i2<cpts; i2++) b[i2+npts] = b2[i2];

			   /* Compute A22' = A22 - A21 * A11_inv * A12 */
                           for(j2=0; j2<cpts; j2++)
			      for(i2=0; i2<npts; i2++)  M[j2][i2] = A12[i2][j2];
			      ...
			      {
				 sum = 0.0;
				 for(k2=0; k2<npts; k2++) sum += A21[i2][k2]*M[j2][k2];
				 A22[i2][j2] -= sum;
			      }
			   ...
			   /* Compute w_tmp where A11*w_tmp = (b1 - A12*b2) */
                           if(npts > 0)
                           {
                              ...
                              {
                                 sum = 0.0;
			         for(k2=0; k2<cpts; k2++) sum += A12[i2][k2]*b2[k2];
                                 w_tmp[i2] = b[i2] - sum;
                              }
			 ...
			 *--------------------------------------------------*/
			cmean = 0.0;
			for(m=0; m<npts+cpts; m++) cmean += w_tmp[m]*value[m];
            ...
            imin = rpx-iLxp1; if(imin < -iLx) imin += iLxp1;
            jmin = rpy-iLyp1; if(jmin < -iLy) jmin += iLyp1;
            kmin = rpz-iLzp1; if(kmin < -iLz) kmin += iLzp1;
   ...
   for(i=0; i<iLxyz; i++) tfree(rand_path[i]);
   tfree(rand_path);

   for (i=0; i<nLx; i++)
   {
     for (j=0; j<nLy; j++) tfree(cov[i][j]);
     tfree(cov[i]);
   }
   tfree(cov);
