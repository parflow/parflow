            ...
            BCStructPatchLoop(i, j, k, fdir, ival, bc_struct, ipatch, is,
	    {
		  ...
		  case -1:
		     ...
		     u_new = ffx * 2.0 * (diff/dx) * rpp[ip];
                     u_new+= ffx * 2.0 * RPMean(0.0, xvp[ip-1], hcwp[ip]*tp[ip],hcwp[ip]*value) * xvp[ip-1];
		     break;
		  case  1:
		     dir = 1;
   		     diff  = tp[ip] - tp[ip+1];
		     ...
		     diff = tp[ip] - value;
		     u_new = ffx * 2.0 * (diff/dx) * rpp[ip];
                     u_new+= ffx * 2.0 * RPMean(0.0, xvp[ip], hcwp[ip+1]*value, hcwp[ip]*tp[ip]) * xvp[ip];
		     break;
		  }
	       }
	       else if (fdir[1])
		  ...
		  case -1:
		     ...
		     u_new = ffy * 2.0 * (diff / dy) * rpp[ip];
                     u_new+= ffy * 2.0 * RPMean(0.0, yvp[ip-sy_p], hcwp[ip]*tp[ip],hcwp[ip]*value) * yvp[ip-sy_p];
		     break;
		  case  1:
		     dir = 1;
   		     diff  = tp[ip] - tp[ip+sy_p];
		     ...
		     diff = tp[ip] - value;
		     u_new = ffy * 2.0 *(diff/dy) * rpp[ip];
                     u_new+= ffy * 2.0 * RPMean(0.0, yvp[ip], hcwp[ip+sy_p]*value, hcwp[ip]*tp[ip]) * yvp[ip];
		     break;
		  }
	       }
	       else if (fdir[2])
		  ...
		  case -1:
		     ...
		     u_old = ffz * diff/dz * RPMean(tp[ip-sz_p], tp[ip], rpp[ip-sz_p], rpp[ip]); 
                     u_old-= ffz * RPMean(0.0, zvp[ip-sz_p], hcwp[ip]*tp[ip],hcwp[ip-sz_p]*tp[ip-sz_p]) * yvp[ip-sz_p];

		     diff = value - tp[ip];
		     u_new = ffz * 2.0 * (diff/dz) * rpp[ip];
                     u_new+= ffz * 2.0 * RPMean(0.0, yvp[ip-sz_p], hcwp[ip]*tp[ip],hcwp[ip]*value) * yvp[ip-sz_p];
		     break;
		     }   /* End case -1 */
		  case  1:
		     {
		     ...
		     diff = tp[ip] - value;
		     u_new = ffz * 2.0 * (diff/dz) * rpp[ip];
                     u_new+= ffz * 2.0 * RPMean(0.0, zvp[ip], hcwp[ip+sz_p]*value, hcwp[ip]*tp[ip]) * zvp[ip];
		     break;
		     }   /* End case 1 */
		  }
	       }
