...
//   double      press[2][1][390],pressbc[400][1],xslope[58][30],yslope[58][30];
   ...
   Subvector   *p_sub, *t_sub, *d_sub, *v_sub, *od_sub, *s_sub, *os_sub, *po_sub, *op_sub, *ss_sub, *et_sub;
   ...
   double      *pp, *tp, *odp, *sp, *osp, *pop, *fp, *dp, *vp, *rpp, *opp, *ss, *et;
  ...
  fflush(NULL);
      ...
      ForSubgridI(is, GridSubgrids(grid))
   {
      ...
      BoxLoopI1(i, j, k, ix, iy, iz, nx, ny, nz,
		ip, nx_f, ny_f, nz_f, 1, 1, 1,
	     {
			 fp[ip] += (ss[ip]/gravity)*vol*(pp[ip]*sp[ip] - opp[ip]*osp[ip]);
                         //press[i][j][k]=pp[ip];
	     });
   }
#endif
	 ...
	 case DirichletBC:
	 {
            BCStructPatchLoop(i, j, k, fdir, ival, bc_struct, ipatch, is,
	    {
	       ...
	       tp[ip + fdir[0]*1 + fdir[1]*sy_p + fdir[2]*sz_p] = value;
               //printf("%d \n",(ip + fdir[0]*1 + fdir[1]*sy_p + fdir[2]*sz_p));
...
//	   if(k==0) printf("i, j, t, io, qy %d %d %d %d %e\n",i,j,t,io,pp[ip]);
		...
		u_right = ffx * PMean(pp[ip], pp[ip+1], 
		              ...
		              * (diff / dx )
		              * RPMean(pp[ip], pp[ip+1], rpp[ip]*dp[ip]/vp[ip],
				       rpp[ip+1]*dp[ip+1]/vp[ip+1]);
		...
		u_front = ffy * PMean(pp[ip], pp[ip+sy_p], permyp[ip], 
				      permyp[ip+sy_p])
		              * (diff / dy )
		              * RPMean(pp[ip], pp[ip+sy_p], rpp[ip]*dp[ip]/vp[ip],
				       rpp[ip+sy_p]*dp[ip+sy_p]/vp[ip+sy_p]);
                yvp[ip] = 1.0/(ffy) * u_front;
	 ...
	 case DirichletBC:
	 {
            BCStructPatchLoop(i, j, k, fdir, ival, bc_struct, ipatch, is,
	    {
		  ...
		  case -1:
		     ...
		     u_old = ffx 
		       ...
		       * RPMean(pp[ip-1], pp[ip], 
				rpp[ip-1]*dp[ip-1]/vp[ip-1], rpp[ip]*dp[ip]/vp[ip]); 
		     diff = value - pp[ip];
		  ...
		  case  1:
		     ...
		     u_old = ffx 
		       ...
		       * RPMean(pp[ip], pp[ip+1],
				rpp[ip]*dp[ip]/vp[ip], rpp[ip+1]*dp[ip+1]/vp[ip+1]); 
		     diff = pp[ip] - value;
		 ...
		 printf("Right BC u_new %12.8f u_old %12.8f value %12.8f "
		  ...
		  case -1:
		     ...
		     u_old = ffy 
		       ...
		       * RPMean(pp[ip-sy_p], pp[ip], 
				rpp[ip-sy_p]*dp[ip-sy_p]/vp[ip-sy_p], rpp[ip]*dp[ip]/vp[ip]); 
		     diff =  value - pp[ip];
		     u_new = RPMean(value, pp[ip], 
				    rpp[ip-sy_p]*dp[ip-sy_p]/vp[ip-sy_p], rpp[ip]*dp[ip]/vp[ip]);
                     yvp[ip-sy_p] = u_new * permxp[ip] * 2.0 * diff/(dy);
		     break;
		  case  1:
		     dir = 1;
   		     diff  = pp[ip] - pp[ip+sy_p];
		     u_old = ffy 
		       ...
		       * RPMean(pp[ip], pp[ip+sy_p], 
				rpp[ip]*dp[ip]/vp[ip], rpp[ip+sy_p]*dp[ip+sy_p]/vp[ip+sy_p]);
		     diff = pp[ip] - value;
		     u_new = RPMean(pp[ip], value,
				    rpp[ip]*dp[ip]/vp[ip], rpp[ip+sy_p]*dp[ip+sy_p]/vp[ip+sy_p]);
		  ...
		  u_new = u_new * ffy * permyp[ip] * 2.0 * (diff/dy);
		  /*
	       if ((k == 0) && (i == 0) && (j == 0))
   ...
   {         
      ...
      p_sub = VectorSubvector(pressure, is); 
      s_sub = VectorSubvector(temperature, is); 
      f_sub = VectorSubvector(fval, is);
