...
/*--------------------------------------------------------------------------
 * KinsolPCTemperatureInitInstanceXtra
 *--------------------------------------------------------------------------*/

PFModule  *KinsolPCTemperatureInitInstanceXtra(problem, grid, problem_data,  
				    temp_data, temperature, pressure, saturation, 
				    density, heat_capacity_water, heat_capacity_rock, 
...
double        time;
{
   ...
   else if (temperature!= NULL)
   {
      PFModuleInvoke(void, (instance_xtra -> discretization),
		  (temperature, &PC, pressure, saturation, density, heat_capacity_water, heat_capacity_rock, 
                   x_velocity, y_velocity, z_velocity, problem_data, dt, 
		   time, pc_matrix_type));
...
/*--------------------------------------------------------------------------
 * KinsolPCTemperatureFreeInstanceXtra
 *--------------------------------------------------------------------------*/

void  KinsolPCTemperatureFreeInstanceXtra()
{
   PFModule      *this_module   = ThisPFModule;
   InstanceXtra  *instance_xtra = PFModuleInstanceXtra(this_module);
...
/*--------------------------------------------------------------------------
 * KinsolPCTemperatureNewPublicXtra
 *--------------------------------------------------------------------------*/
...
PFModule  *KinsolPCTemperatureNewPublicXtra(char *name, char *pc_name)
{
   ...
   public_xtra -> discretization = PFModuleNewModule(TemperatureJacobianEval, ());
...
/*-------------------------------------------------------------------------
 * KinsolPCTemperatureFreePublicXtra
 *-------------------------------------------------------------------------*/

void  KinsolPCTemperatureFreePublicXtra()
{
   PFModule    *this_module   = ThisPFModule;
   PublicXtra  *public_xtra   = PFModulePublicXtra(this_module);
...
/*--------------------------------------------------------------------------
 * KinsolPCTemperatureSizeOfTempData
 *--------------------------------------------------------------------------*/

int  KinsolPCTemperatureSizeOfTempData()
{
   PFModule             *this_module   = ThisPFModule;
   InstanceXtra         *instance_xtra = PFModuleInstanceXtra(this_module);
   PFModule             *precond       = (instance_xtra -> precond);
