...
PFModule  *YSlopeInitInstanceXtra(grid)
Grid    *grid;
{
   PFModule      *this_module   = ThisPFModule;
   PublicXtra    *public_xtra   = PFModulePublicXtra(this_module);
   ...
   if (public_xtra -> type == 2)
   {
	   ...
	   (instance_xtra -> temp_data) = talloc(double, SizeOfVector(dummy2 -> sy_values));
	   SetTempVectorData((dummy2 -> sy_values),  (instance_xtra -> temp_data));

	   ReadPFBinary((dummy2 -> filename),(dummy2 -> sy_values));
   
/*   dat = NewVector( grid, 1, 2);
   InitVector(dat, 0.0);
   dat = dummy2 -> sy_values;
   dat_sub = VectorSubvector(dat,0);
   dat_dat = SubvectorData(dat_sub);
 
   for (k = 0; k < 392; k++) {
     for (j = 0; j < 32; j++) {
       for (i = 0; i < 60; i++) {
       ip = SubvectorEltIndex(dat_sub,i,j,k);
       value[i][j][k] = dat_dat[ip];
       }
     }
   }
