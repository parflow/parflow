...
/*  This routine evaluates the temperature jacobian based on the current 
    temperature values.  */

void    TemperatureJacobianEval(temperature, ptr_to_J, pressure, saturation, density, viscos,  
				 ...
				 full Jacobian */
{
   ...
   Vector      *viscosity_p_der   = (instance_xtra -> viscosity_p_der);
  ...
  //double      press[58][30][360],pressbc[58][30],xslope[58][30],yslope[58][30],mans[58][30];
   ...
   Vector      *perm_h2o_x        = ProblemDataPermeabilityX(problem_data);
   Vector      *perm_h2o_y        = ProblemDataPermeabilityY(problem_data);
   Vector      *perm_h2o_z        = ProblemDataPermeabilityZ(problem_data);
   ...
   Subvector   *p_sub, *d_sub, *s_sub, *po_sub, *rp_sub, *ss_sub, *hc_sub;
   ...
   double      *cp, *wp, *ep, *sop, *np, *lp, *up, *op, *ss, *hcp;
   ...
   /* sk: I need this vector in the ad hoc approach; it must be 1.0 */
   perm_heat = NewVector( grid, 1, 1);
   InitVectorAll (perm_heat, 1.0); 

   rel_perm = NewVector( grid, 1, 1); 
   InitVectorAll(rel_perm, 0.0);
   rel_perm_der = NewVector( grid, 1, 1);
   InitVectorAll(rel_perm_der, 0.0);
   ...
   PFModuleInvoke(void, saturation_module, (saturation_der, pressure, 
	 ...
	 BoxLoopI3(i, j, k, ix, iy, iz, nx, ny, nz,
		   ...
		   {
                       /* Heat: contribution from rock and pore water */
                       cp[im] = (1.0-pop[ipo]) * hcp[ipo] * vol; 
                       cp[im] += pop[ipo] * vol * (sdp[iv]*dp[iv]+sp[iv]*ddp[iv]);
		   });
      }   /* End subgrid loop */
#endif
   ...
   /* Calculate rel_perm and rel_perm_der */
     ...
     permeability_x = perm_heat;
     permeability_y = perm_heat;
     permeability_z = perm_heat;
    
     PFModuleInvoke(void, thermal_conductivity,
                    (rel_perm, temperature, saturation, gravity, problem_data,
                     CALCFCN));

     PFModuleInvoke(void, thermal_conductivity,
                    (rel_perm_der, temperature, saturation_der, gravity, problem_data,
                     CALCDER));
   ...
   if (symm_part)
   {
      ...
      /*  direction boundary sepcie by adding in the nonsymmetric part. */
...
int          symmetric_jac;
{
   ...
   if ( grid != NULL )
   {
	 ...
	 FreeMatrix(instance_xtra -> J);
         ...
         FreeVector(instance_xtra -> viscosity_p_der);
      }
	 ...
	 (instance_xtra -> J) = NewMatrix(grid, NULL, stencil, OFF, stencil);
      ...
      (instance_xtra -> viscosity_p_der) = NewVector(grid, 1, 1);
   }
   ...
   if ( temp_data != NULL )
   {
      ...
      InitVectorAll(instance_xtra -> viscosity_p_der, 0.0);
   }
