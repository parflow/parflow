...
/*--------------------------------------------------------------------------
 * KinsolPCPressureInitInstanceXtra
 *--------------------------------------------------------------------------*/

PFModule  *KinsolPCPressureInitInstanceXtra(problem, grid, problem_data,  
				    temp_data, pressure, temperature, saturation, 
				    density, viscosity, dt, time)
Problem      *problem;
Grid         *grid;
...
double        time;
{
   ...
   else if (pressure!= NULL)
   {
      PFModuleInvoke(void, (instance_xtra -> discretization),
		  (pressure, &PC, temperature, saturation, density, viscosity, problem_data, dt, 
		   time, pc_matrix_type));
...
/*--------------------------------------------------------------------------
 * KinsolPCPressureFreeInstanceXtra
 *--------------------------------------------------------------------------*/

void  KinsolPCPressureFreeInstanceXtra()
{
   PFModule      *this_module   = ThisPFModule;
   InstanceXtra  *instance_xtra = PFModuleInstanceXtra(this_module);
...
/*--------------------------------------------------------------------------
 * KinsolPCPressureNewPublicXtra
 *--------------------------------------------------------------------------*/

PFModule  *KinsolPCPressureNewPublicXtra(char *name, char *pc_name)
{
   PFModule      *this_module   = ThisPFModule;
...
/*-------------------------------------------------------------------------
 * KinsolPCPressureFreePublicXtra
 *-------------------------------------------------------------------------*/

void  KinsolPCPressureFreePublicXtra()
{
   PFModule    *this_module   = ThisPFModule;
   PublicXtra  *public_xtra   = PFModulePublicXtra(this_module);
...
/*--------------------------------------------------------------------------
 * KinsolPCPressureSizeOfTempData
 *--------------------------------------------------------------------------*/

int  KinsolPCPressureSizeOfTempData()
{
   PFModule             *this_module   = ThisPFModule;
   InstanceXtra         *instance_xtra = PFModuleInstanceXtra(this_module);
   PFModule             *precond       = (instance_xtra -> precond);
