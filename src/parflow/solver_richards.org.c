...
/* sk: Vector that contains the sink terms from the land surface model*/ 
  ...
  Vector       *fval;
  Subgrid      *subgrid;
  Subvector    *p_sub, *s_sub, *f_sub, *et_sub, *m_sub;
  double       *pp,*sp, *fp, *et, *ms;
...
//	  outflow_log  = talloc(double, 10);
  ...
  //CALL_CLM_INI(nx,ny,nt,clm1dp,drvdecp,tiledecp,clmgridp);
  ...
  do  /* while take_more_time_steps */
  {
	...
	if (converged)
	{
	   ...
	   PFVCopy(pressure,   old_pressure);
           ...
           { 
            ...
            // CALL_CLM_LSM(pp,sp,et,ms,dt_old,t_old,nx,ny,nz,nx_f,ny_f,nz_f,ip,p,q,r);
           }
	}
	else  /* Not converged, so decrease time step */
	{
	   ...
	   PFVCopy(old_pressure,   pressure);
	   if(!amps_Rank(amps_CommWorld)) 
	      printf("Decreasing step size for step taken at time %12.4e.\n", 
		     t);
           /* sk: call to the land surface model/subroutine*/
	}
	...
	retval = PFModuleInvoke(int, nonlin_solver, 
				(pressure, density, old_density, saturation, 
...
PFModule *SolverRichardsInitInstanceXtra()
{
   ...
   (instance_xtra -> problem_data) = NewProblemData(grid);
