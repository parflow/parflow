...
subroutine drv_g2clm(casc,u,drv,grid,tile,clm)
...
!=========================================================================
  ...
  use casctype            !@ Overland flow variables
...
!@ Overland routing (read into 2-D casc module variables)
  if (grid(c,r)%sdep            /= u) casc%sdep      =grid(c,r)%sdep
  if (grid(c,r)%manning         /= u) casc%manning   =grid(c,r)%manning 
  if (grid(c,r)%W               /= u) casc%W         =grid(c,r)%W * 1000.0d0 !@ conversion from [m] to [mmm]
