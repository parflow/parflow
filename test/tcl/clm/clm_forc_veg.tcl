...
if ![pftestFile clm_forc_veg.out.perm_x.pfb "Max difference in perm_x" $sig_digits $correct_output_dir] {
    set passed 0
}
if ![pftestFile clm_forc_veg.out.perm_y.pfb "Max difference in perm_y" $sig_digits $correct_output_dir] {
    set passed 0
}
if ![pftestFile clm_forc_veg.out.perm_z.pfb "Max difference in perm_z" $sig_digits $correct_output_dir] {
    set passed 0
}

for {set i 0} { $i <= 5 } {incr i} {
    set i_string [format "%05d" $i]
    if ![pftestFile clm_forc_veg.out.press.$i_string.pfb "Max difference in Pressure for timestep $i_string" $sig_digits $correct_output_dir] {
    set passed 0
    }
    if ![pftestFile clm_forc_veg.out.satur.$i_string.pfb "Max difference in Saturation for timestep $i_string" $sig_digits $correct_output_dir] {
    set passed 0
    }
}

set mask [pfload clm_forc_veg.out.mask.pfb]
set top [Parflow::pfcomputetop $mask]

pfsave $top -pfb "clm_forc_veg.out.top_index.pfb"

set data [pfload clm_forc_veg.out.press.00000.pfb]
set top_data [Parflow::pfextracttop $top $data]

pfsave $data -pfb "clm_forc_veg.out.press.00000.pfb"
pfsave $top_data -pfb "clm_forc_veg.out.top.press.00000.pfb"
...
if ![pftestFile clm_forc_veg.out.top_index.pfb "Max difference in top_index" $sig_digits $correct_output_dir] {
    set passed 0
}

if ![pftestFile clm_forc_veg.out.top.press.00000.pfb "Max difference in top_clm.out.press.00000.pfb" $sig_digits $correct_output_dir] {
    set passed 0
}
...
if $passed {
    puts "clm_forc_veg : PASSED"
} {
    puts "clm_forc_veg : FAILED"
}
