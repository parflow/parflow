...
pfset Solver.WriteSiloOverlandCellOutflow               True
pfset Solver.WriteSiloOverlandFaceFlow                  True
...
for {set i 0} {$i <= 19} {incr i} {
	...
	# Compute surface runoff using the TCL methods
	...
	set filename [format "%s.out.overland_cell_outflow.%05d.silo" $runname  $i]
	set cell_outflow [pfload $filename]
	set boundary_outflow [pfextracttopboundary $top $cell_outflow]

	puts [format "Surface boundary outflow sum %.16e" [pfsum $boundary_outflow]]

	set total_surface_runoff2 0.0
	# Compute surface runoff from files generated by ParFlow.
	foreach face "ke kw kn ks" {
	    set filename [format "%s.out.overland_face_flow_%s.%05d.silo" $runname $face $i]

	    set face_flow [pfload $filename]

	    set surface_runoff2 [pfextracttopboundary $top $face_flow]
	    set partial_sum [pfsum $surface_runoff2]
	    puts [format "Surface runoff %s %.16e" $face $partial_sum]
	    set total_surface_runoff2 [expr $total_surface_runoff2 + [pfsum $surface_runoff2]]
	}
