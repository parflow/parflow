...
def remove_restart_files():
    ...
    """
    for path in glob.glob('default_richards_restart.out.press.*') + glob.glob('default_richards_restart.out.satur.*'):
        os.remove(path)
...
#---------------------------------------------------------
default_richards.GeomInput.Names = 'domain_input background_input source_region_input concen_region_input'
...
shutil.copy(os.path.join(new_output_dir_name, 'default_richards.out.press.00010.pfb'), os.path.join(new_output_dir_name, initial_pressure))
