...
run_name = "clm_evaptrans_transient"
...
new_output_dir_name = get_absolute_path("test_output/clm_evaptrans_transient")
...
cp("$PF_SRC/test/input/evaptrans.00000.pfb", new_output_dir_name)
cp("$PF_SRC/test/input/evaptrans.00001.pfb", new_output_dir_name)
cp("$PF_SRC/test/input/evaptrans.00002.pfb", new_output_dir_name)
cp("$PF_SRC/test/input/evaptrans.00003.pfb", new_output_dir_name)
cp("$PF_SRC/test/input/evaptrans.00004.pfb", new_output_dir_name)
...
clm.Solver.PrintSlopes = True
clm.Solver.PrintMannings = True
...
# -----------------------------------------------------------------------------
for i in range(5):
    clm.dist(new_output_dir_name + f"/evaptrans.{i:05d}.pfb")
...
for i in range(6):
    ...
    if i > 0:
        ...
        filename = f"/{run_name}.out.evaptrans.{timestep}.pfb"
        if not pf_test_file(
            new_output_dir_name + filename,
            correct_output_dir_name + filename,
            f"Max difference in EvapTrans for timestep {timestep}",
        ):
            passed = False
