# -----------------------------------------------------------------------------
# CLM Clean Defaults - Bit-for-Bit Verification Test
#
# This test verifies that drv_clmin_clean.dat (with dead/unused parameters
# removed) produces BIT-FOR-BIT identical output to the original drv_clmin.dat.
#
# Removed parameters (all dead code — no effect on computation):
#   metf1d          — Read but unused in ParFlow-coupled mode
#   qflx_tran_vegmx — Not read by Fortran reader at all
#
# Parameters not present in original file but read by Fortran (also dead):
#   hkdepth, wtfact, trsmx0, smpmax, scalez, pondmx
#   These stay at NaN init but never enter active computation paths.
#
# Uses SAME correct_output/clm_snow_defaults/ reference as the original test.
# If this test passes, the clean file is a safe drop-in replacement.
# -----------------------------------------------------------------------------
...
# Use CLEAN driver file instead of original
cp("$PF_SRC/test/tcl/clm/drv_clmin_clean.dat", dir_name + "/drv_clmin.dat")
...
# Same as clm_snow_defaults.py to ensure bit-for-bit match
# -----------------------------------------------------------------------------
...
# Compare against SAME reference as clm_snow_defaults
correct_output_dir_name = get_absolute_path("../../correct_output/clm_snow_defaults")

clm.run(working_directory=dir_name)

# -----------------------------------------------------------------------------
# Tests - Compare pressure, saturation, and CLM output
# Output files use this test's run_name, reference files use clm_snow_defaults
# -----------------------------------------------------------------------------
...
for i in range(6):
    ...
    # Compare pressure
    test_file = f"/{run_name}.out.press.{timestep}.pfb"
    ref_file = f"/clm_snow_defaults.out.press.{timestep}.pfb"
    if not pf_test_file(
        dir_name + test_file,
        correct_output_dir_name + ref_file,
        f"Max difference in Pressure for timestep {timestep}",
    ):
        passed = False

    # Compare saturation
    test_file = f"/{run_name}.out.satur.{timestep}.pfb"
    ref_file = f"/clm_snow_defaults.out.satur.{timestep}.pfb"
    if not pf_test_file(
        dir_name + test_file,
        correct_output_dir_name + ref_file,
    ...
    # Compare CLM output (starts at timestep 1)
    if i > 0:
        test_file = f"/{run_name}.out.clm_output.{timestep}.C.pfb"
        ref_file = f"/clm_snow_defaults.out.clm_output.{timestep}.C.pfb"
        if not pf_test_file(
            dir_name + test_file,
            correct_output_dir_name + ref_file,
