...
def test_output(file_name):
  ...
  with open(get_absolute_path(f'{file_name}.yaml')) as new, \
      open(get_absolute_path('LW_test_ref.yaml.ref')) as ref:
...
print('Clearing:')
SubsurfacePropertiesBuilder(LW_Test) \
  .load_csv_file('LW_test_data_clear.csv') \
  .apply()
...
SubsurfacePropertiesBuilder(LW_Test) \
  .load_csv_file('LW_test_data.csv') \
  .apply() \
...
SubsurfacePropertiesBuilder(LW_Test) \
    .load_txt_file('LW_test_data.txt') \
    .apply() \
    .print_as_table()
print('+'*40)
...
SubsurfacePropertiesBuilder(LW_Test) \
    .load_txt_file('LW_test_data_transposed.txt') \
    .apply() \
    .print()
print('+'*40)
