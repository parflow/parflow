...
assert (run.value('/Geom/background/Porosity/Value') ==
        run.Geom.background.Porosity.Value)
assert (background.value('/Geom/background/Porosity/Value')
        == run.Geom.background.Porosity.Value)
assert (background.value('tce/Retardation/Type') ==
        run.Geom.background.tce.Retardation.Type)
assert (background.value('./tce/Retardation/Type') ==
        run.Geom.background.tce.Retardation.Type)
assert (background.value('tce/Retardation/Rate') ==
        run.Geom.background.tce.Retardation.Rate)
assert (background.value('../concen_region/Lower/Z') ==
        run.Geom.concen_region.Lower.Z)
...
assert (run.Solver.details('.').get('default') ==
        run.Solver['_details_']['_value_']['default'])
assert run.Solver.CLM.details('..') is run.Solver['_details_']['_value_']
assert (run.Solver.CLM.details('..').get('default') ==
        run.Solver['_details_']['_value_']['default'])
assert (run.details('Solver/TerrainFollowingGrid').get('default') ==
...
assert (background.doc('./Lower/X') ==
        background.Lower._details_['X']['help'] + '\n')
assert (background.doc('./Lower/../..//./background/Upper/Y/.') ==
