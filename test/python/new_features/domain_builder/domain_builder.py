...
DomainBuilder(drich) \
    .water('background') \
    .variably_saturated() \
    .box_domain('domain_input', 'domain',
                bounds=[
                  -10.0, 150.0,   # x range
                  10.0, 170.0,    # y range
                  1.0, 9.0        # z range
    ...
    .box_domain('background_input', 'background',
                bounds=[
                  -99999999.0, 99999999.0,
                  -99999999.0, 99999999.0,
                  -99999999.0, 99999999.0
                ]) \
    .box_domain('source_region_input', 'source_region',
                bounds=[
                  65.56, 74.44,
                  79.34, 89.99,
                  4.5, 5.5
                ]) \
    .box_domain('concen_region_input', 'concen_region',
                bounds=[
                  60.0, 80.0,
                  80.0, 100.0,
                  4.0, 6.0
                ]) \
    .slopes_mannings('domain', slope_x=0.0, slope_y=0.0, mannings=0.0) \
    .homogeneous_subsurface('domain',
    ...
    .homogeneous_subsurface('background',
                            perm=4.0,
                            porosity=1.0,
                            isotropic=True
                            ) \
    .zero_flux(patches='front back bottom top',
               cycle_name='constant',
               interval_name='alltime')
...
with open(get_absolute_path('drich.yaml')) as new, \
    open(get_absolute_path('default_richards.yaml.ref')) as ref:
