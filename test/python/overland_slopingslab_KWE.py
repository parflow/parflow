...
for x_slope, y_slope, name in zip(x_slopes, y_slopes, names):
    ...
    new_output_dir_name = get_absolute_path(
        "test_output/" + f"run_name_nonsymmetric_preconditioner"
    )
    mkdir(new_output_dir_name)
    overland.run(working_directory=new_output_dir_name)
    if runcheck == 1:
    ...
    print(f"Running {run_name} Jacobian True")
    new_output_dir_name = get_absolute_path(
        "test_output/" + f"run_name_overland_kin_jacobian_true"
    )
    mkdir(new_output_dir_name)
    overland.run(working_directory=new_output_dir_name)
    if runcheck == 1:
    ...
    new_output_dir_name = get_absolute_path(
        "test_output/"
        + f"run_name_overland_kin_jacobian_true_nonsymmetric_preconditioner"
    )
    mkdir(new_output_dir_name)
    overland.run(working_directory=new_output_dir_name)
    if runcheck == 1:
