...
#
#  Variant of crater2D_vangtable_linear_sat.py with a STEEP retention curve
#  (alpha = 3.548, n = 4.162: a real CONUS2.1 soil unit) and the Linear
#  saturation lookup table.  Regression test for the saturation linear-table
#  CALCDER: with a smooth interpolated analytic dS/dh (inconsistent with the
#  piecewise-linear CALCFCN at first order in the sample interval) the
#  Jacobian storage term is tens of percent wrong at the wet front on soils
#  this steep, and Newton degenerates to linear convergence.  The consistent
#  segment-slope CALCDER converges normally.  The gentle curves in the other
#  crater2D table tests (alpha = 0.01) cannot see this bug.
#
#  RelPerm is left as direct analytic evaluation (no table) so the test
#  isolates the saturation-table path in the storage term.
#
...
run_name = "crater2D_vangtable_linear_sat_steep"
...
dir_name = get_absolute_path("test_output/crater_vl_sat_steep")
mkdir(dir_name)

cp("$PF_SRC/test/input/crater2D.pfsol", dir_name)

# ---------------------------------------------------------
# Controls for the Van Genuchten curves used later.
# ---------------------------------------------------------
# Steep CONUS2.1 sand-type curve (East River soil unit s1).  With 20000
# uniform sample points over 300 m of head (1.5 cm spacing) the retention
# knee near h = 1/alpha = 0.28 m is under-resolved enough that an
# inconsistent CALCDER visibly degrades Newton, which is the point.
...
crater.Geom.zone4.Perm.Value = 0.48033
...
crater.TimingInfo.StopTime = 100.0
crater.TimingInfo.DumpInterval = 50.0
...
# -----------------------------------------------------------------------------
# Relative Permeability (direct analytic - no table, to isolate the
# saturation-table path)
# -----------------------------------------------------------------------------
...
for zone in Zones.split():
    zone_geom = getattr(crater.Geom, zone)
    zone_geom.RelPerm.Alpha = VG_alpha
    zone_geom.RelPerm.N = VG_N

# ---------------------------------------------------------
# Saturation (Linear lookup table on every zone)
# ---------------------------------------------------------
...
for zone in Zones.split():
    zone_geom = getattr(crater.Geom, zone)
    zone_geom.Saturation.Alpha = VG_alpha
    zone_geom.Saturation.N = VG_N
    zone_geom.Saturation.SRes = VG_sres
    zone_geom.Saturation.SSat = 1.0
    zone_geom.Saturation.NumSamplePoints = VG_points
    zone_geom.Saturation.MinPressureHead = -300
    zone_geom.Saturation.InterpolationMethod = VG_interpolation_method
...
# Uniform mid-knee suction, far from hydrostatic equilibrium: every cell
# drains through the steep part of the retention curve, so the small-dt
# storage term (and its CALCDER) controls Newton everywhere.
crater.ICPressure.Type = "Constant"
crater.ICPressure.GeomNames = "domain"

crater.Geom.domain.ICPressure.Value = -0.5
...
# MaxIter is generous so the inconsistent-CALCDER failure mode shows up as
# a large iteration count (caught by NNI_MAX) rather than a step failure.
crater.Solver.Nonlinear.MaxIter = 100
...
# Set runcheck = 0 for the initial sanity run (correct_output PFBs not yet
# generated). After inspecting test_output and copying PFBs to
# test/correct_output/, set runcheck = 1 to enable regression comparison.
runcheck = 1

# Newton-efficiency guard: the whole point of this test.  An inconsistent
# saturation-table CALCDER converges (so output comparisons alone can pass)
# but needs several times more nonlinear iterations.
total_nni = 0
kinsol_log = f"{new_output_dir_name}/{run_name}.out.kinsol.log"
with open(kinsol_log) as f:
    for line in f:
        if line.startswith("Nonlin. Its.:"):
            total_nni += int(line.split()[2])

print(f"{run_name} : total nonlinear iterations = {total_nni} (max {NNI_MAX})")
passed = total_nni <= NNI_MAX
...
if not runcheck:
    ...
    if not passed:
        print(f"{run_name} : FAILED (nonlinear iteration budget exceeded)")
        sys.exit(1)
    sys.exit(0)
