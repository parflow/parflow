...
#-----------------------------------------------------------------------------
# Verify against published Molins et al. 2025 (GMD 18:3241) Fig. 2(a) tracer
# profile. Reference output generated by this port; non-reactive front at 50 m.
#-----------------------------------------------------------------------------
source ../tcl/pftest.tcl
set passed 1
set sig_digits 6

if ![pftestFile tracer_pf.out.PrimaryMobile.00.tracer.00005.pfb "Max difference in tracer" $sig_digits "correct_output"] {
    set passed 0
}

if $passed {
    puts "tracer_crunch : PASSED"
} {
    puts "tracer_crunch : FAILED"
}
