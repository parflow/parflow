...
if(NOT (NETCDF_DIR OR NETCDF_INCLUDE_DIR OR NetCDF_LIBRARY))
    ...
    if(NOT NETCDF_INCLUDE_DIRS)
        message(
            FATAL_ERROR
            "NetCDF header netcdf.h not found in ${NETCDF_DIR}/include; could not find NetCDF install"
        )
    endif()
    ...
    if(NOT NETCDF_LIBRARIES)
        message(
            FATAL_ERROR
            "NetCDF library not found in ${NETCDF_DIR}/lib; could not find NetCDF install"
        )
    endif()
endif()
...
if(NETCDF_PAR_H)
    ...
    message(
        WARNING
        "NetCDF parallel header file ${include_dir}/netcdf_par.h not found."
    )
endif()
...
if(NETCDF_INCLUDE_DIRS AND NOT NETCDF_VERSION)
    file(
        STRINGS "${NETCDF_INCLUDE_DIRS}/netcdf_meta.h"
        _netcdf_version_lines
        REGEX "#define[ \t]+NC_VERSION_(MAJOR|MINOR|PATCH|NOTE)"
    )
    string(
        REGEX REPLACE ".*NC_VERSION_MAJOR *\([0-9]*\).*"
        "\\1"
        _netcdf_version_major
        "${_netcdf_version_lines}"
    )
    string(
        REGEX REPLACE ".*NC_VERSION_MINOR *\([0-9]*\).*"
        "\\1"
        _netcdf_version_minor
        "${_netcdf_version_lines}"
    )
    string(
        REGEX REPLACE ".*NC_VERSION_PATCH *\([0-9]*\).*"
        "\\1"
        _netcdf_version_patch
        "${_netcdf_version_lines}"
    )
    string(
        REGEX REPLACE ".*NC_VERSION_NOTE *\"\([^\"]*\)\".*"
        "\\1"
