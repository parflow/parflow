...
macro(pf_exec_check cmd)
    ...
    execute_process(
        ...
        OUTPUT_VARIABLE joined_stdout_stderr
        ERROR_VARIABLE joined_stdout_stderr
    )
...
macro(pf_test_clean)
    ...
    file(
        ...
        default_single.out
        water_balance.out
        default_overland.out
        LW_var_dz_spinup.out
        test.log.*
        richards_hydrostatic_equalibrium.out
        core.*
        samrai_grid.tmp.tcl
        samrai_grid2D.tmp.tcl
        CMakeCache.txt
    )
    if(NOT FILES STREQUAL "")
        file(REMOVE ${FILES})
...
list(APPEND CMD ${PARFLOW_PYTHON} ${PYTHON_TEST_SCRIPT})

# Add topology arguments if provided
if(PYTHON_TEST_ARGS)
    separate_arguments(py_args UNIX_COMMAND "${PYTHON_TEST_ARGS}")
    list(APPEND CMD ${py_args})
endif()

# Set environment variables for valgrind if enabled
if(${PARFLOW_HAVE_MEMORYCHECK})
    set(ENV{PARFLOW_MEMORYCHECK_COMMAND} ${PARFLOW_MEMORYCHECK_COMMAND})
    set(ENV{PARFLOW_MEMORYCHECK_COMMAND_OPTIONS}
        ${PARFLOW_MEMORYCHECK_COMMAND_OPTIONS}
    )
endif()

# Set Python-specific environment variables
set(ENV{PYTHONPATH} "${PYTHONPATH}")
set(ENV{PF_SRC} "${PF_SRC}")
