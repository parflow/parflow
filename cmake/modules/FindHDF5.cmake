...
function(_HDF5_test_regular_compiler_C success version is_parallel)
    ...
    if(NOT ${success} OR NOT EXISTS ${scratch_directory}/compiler_has_h5_c)
    ...
    if(${success})
        file(
            STRINGS ${scratch_directory}/compiler_has_h5_c
            INFO_VER
            REGEX "^INFO:([0-9]+\\.[0-9]+\\.[0-9]+)(-patch([0-9]+))?"
        )
        string(
            REGEX MATCH "^INFO:([0-9]+\\.[0-9]+\\.[0-9]+)(-patch([0-9]+))?"
            INFO_VER
            "${INFO_VER}"
        )
        ...
        execute_process(
            COMMAND ${CMAKE_C_COMPILER} -showconfig
...
function(_HDF5_test_regular_compiler_CXX success version is_parallel)
    ...
    if(NOT ${success} OR NOT EXISTS ${scratch_directory}/compiler_has_h5_cxx)
    ...
    if(${success})
        file(
            STRINGS ${scratch_directory}/compiler_has_h5_cxx
            INFO_VER
            REGEX "^INFO:([0-9]+\\.[0-9]+\\.[0-9]+)(-patch([0-9]+))?"
        )
        string(
        ...
        execute_process(
            COMMAND ${CMAKE_CXX_COMPILER} -showconfig
            OUTPUT_VARIABLE config_output
            ERROR_VARIABLE config_error
            RESULT_VARIABLE config_result
        )
...
function(_HDF5_test_regular_compiler_Fortran success is_parallel)
    if(NOT ${success})
        ...
        if(${success})
            execute_process(
                COMMAND ${CMAKE_Fortran_COMPILER} -showconfig
                OUTPUT_VARIABLE config_output
                ERROR_VARIABLE config_error
                RESULT_VARIABLE config_result
            )
...
macro(
    ...
    exec_program(
        ${HDF5_${language}_COMPILER_EXECUTABLE}
        ARGS
        -show
        ${lib_type_args}
        ${test_file}
    ...
    string(
        REGEX MATCH "HDF5 Version: ([a-zA-Z0-9\\.\\-]*)"
        version_match
        "${config_output}"
    )
    if(version_match)
...
if(NOT HDF5_FOUND AND NOT HDF5_ROOT)
    ...
    if(HDF5_FOUND)
        ...
        foreach(_lang ${HDF5_LANGUAGE_BINDINGS})
            ...
            get_target_property(
                _lang_location
                ${HDF5_${_lang}_TARGET}${_suffix}
                LOCATION
            )
            if(_lang_location)
                set(HDF5_${_lang}_LIBRARY
                    ${_lang_location}
                    CACHE PATH
                    "HDF5 ${_lang} library"
                )
            ...
            if(FIND_HL)
                get_target_property(
                    _lang_hl_location
                    ${HDF5_${_lang}_HL_TARGET}${_suffix}
                    LOCATION
                )
                if(_lang_hl_location)
                    set(HDF5_${_lang}_HL_LIBRARY
                        ${_lang_hl_location}
                        CACHE PATH
...
if(NOT HDF5_FOUND AND NOT HDF5_ROOT)
    ...
    foreach(__lang IN LISTS HDF5_LANGUAGE_BINDINGS)
        ...
        if(HDF5_${__lang}_COMPILER_NO_INTERROGATE)
            ...
            find_program(
                HDF5_${__lang}_COMPILER_EXECUTABLE
                NAMES ${HDF5_${__lang}_COMPILER_NAMES}
                NAMES_PER_DIR
                PATH_SUFFIXES bin Bin
                DOC
                    "HDF5 ${__lang} Wrapper compiler.  Used only to detect HDF5 compile flags."
            )
            ...
            if(HDF5_${__lang}_COMPILER_EXECUTABLE)
                ...
                if(HDF5_${__lang}_RETURN_VALUE EQUAL 0)
                    message(
                        STATUS
                        "HDF5: Using hdf5 compiler wrapper to determine ${__lang} configuration"
                    )
            ...
            HDF5_${__lang}_HL_LIBRARY_NAMES
                    ...
                    set(_HDF5_CMAKE_FIND_LIBRARY_SUFFIXES
                    ...
                    foreach(L IN LISTS HDF5_${__lang}_LIBRARY_NAMES)
                        ...
                        if(HDF5_${__lang}_LIBRARY_${L})
                            list(
                                APPEND HDF5_${__lang}_LIBRARIES
                                ${HDF5_${__lang}_LIBRARY_${L}}
                            )
                        else()
                    ...
                    if(FIND_HL)
                        ...
                        foreach(L IN LISTS HDF5_${__lang}_HL_LIBRARY_NAMES)
                            ...
                            if(HDF5_${__lang}_LIBRARY_${L})
                                list(
                                    APPEND HDF5_${__lang}_HL_LIBRARIES
                                    ${HDF5_${__lang}_LIBRARY_${L}}
                                )
                            else()
                    ...
                    set(CMAKE_FIND_LIBRARY_SUFFIXES
                        ${_HDF5_CMAKE_FIND_LIBRARY_SUFFIXES}
                    )
        ...
        if(HDF5_${__lang}_VERSION)
            ...
            elseif(NOT HDF5_VERSION VERSION_EQUAL HDF5_${__lang}_VERSION)
                message(
                    WARNING
                    "HDF5 Version found for language ${__lang}, ${HDF5_${__lang}_VERSION} is different than previously found version ${HDF5_VERSION}"
                )
            endif()
        endif()
        if(DEFINED HDF5_${__lang}_IS_PARALLEL)
            ...
            elseif(NOT HDF5_IS_PARALLEL AND HDF5_${__lang}_IS_PARALLEL)
                message(
                    WARNING
                    "HDF5 found for language ${__lang} is parallel but previously found language is not parallel."
                )
            elseif(HDF5_IS_PARALLEL AND NOT HDF5_${__lang}_IS_PARALLEL)
                message(
                    WARNING
                    "HDF5 found for language ${__lang} is not parallel but previously found language is parallel."
                )
            endif()
        endif()
    endforeach()
    ...
    foreach(__lang IN LISTS HDF5_LANGUAGE_BINDINGS)
        if(HDF5_${__lang}_FOUND)
            if(NOT HDF5_${__lang}_COMPILER_NO_INTERROGATE)
                ...
                if(FIND_HL)
                    list(
                        APPEND HDF5_HL_LIBRARIES
                        ${HDF5_${__lang}_HL_LIBRARIES}
                    )
                endif()
            endif()
        endif()
...
find_program(
    ...
    NAMES h5diff
    HINTS ${HDF5_ROOT}
    PATH_SUFFIXES bin Bin ${SEARCH_OPTS}
...
if(NOT HDF5_FOUND)
    ...
    set(HDF5_CXX_HL_LIBRARY_NAMES
        hdf5_hl_cpp
        ${HDF5_C_HL_LIBRARY_NAMES}
    ...
    foreach(__lang IN LISTS HDF5_LANGUAGE_BINDINGS)
        ...
        find_path(
            HDF5_${__lang}_INCLUDE_DIR
            ${HDF5_INCLUDE_FILENAME}
            HINTS ${HDF5_ROOT}
            PATHS $ENV{HOME}/.local/include
            PATH_SUFFIXES include Include ${SEARCH_OPTS}
        )
        ...
        foreach(LIB IN LISTS HDF5_${__lang}_LIBRARY_NAMES)
            if(UNIX AND HDF5_USE_STATIC_LIBRARIES)
                ...
                set(THIS_LIBRARY_SEARCH_DEBUG
                    ...
                    ${LIB}_debug
                    lib${LIB}d-static.a
                    lib${LIB}_debug-static.a
                    ${LIB}d-static
                    ${LIB}_debug-static
                )
                set(THIS_LIBRARY_SEARCH_RELEASE
                    lib${LIB}.a
                    ${LIB}
                    lib${LIB}-static.a
                    ${LIB}-static
            ...
            find_library(
                ...
                HINTS ${HDF5_ROOT}
                PATH_SUFFIXES lib Lib ${SEARCH_OPTS}
            )
            find_library(
