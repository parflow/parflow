...
This key must be one of: {\bf SolidFile, IndicatorField, IndicatorFieldNC}, {\bf Box}.
}
\begin{display}\begin{verbatim}
...
names.  For IndicatorField and IndicatorFieldNC types you need to specify the value in the
...
{
For IndicatorField, IndicatorFieldNC and SolidFile geometry inputs this key specifies
...
{
For IndicatorField and IndicatorFieldNC geometry inputs you need to specify the mapping between
...
DirEquilPLinear, FluxConst, FluxVolumetric, PressureFile, FluxFile, OverlandFow, OverlandFlowPFB, SeepageFace, OverlandKinematic, OverlandDiffusive, DeepAquifer} and
...
below). The choice {\bf DeepAquifer}
turns on groundwater interactions with a deep aquifer as described in
\cite{Rahman2018} and \S~\ref{Deep Aquifer}. For this boundary condition,
...
the elevations on each cell, detailed below. NOTE: These last seven types of
boundary condition input is for {\em Richards' equation cases only!}
}
\begin{display}\begin{verbatim}
...
{no default}
This key is required when the {\bf DeepAquifer} boundary condition is chosen.
...
Possible values for this key are {\bf Constant, PFBFile, NCFile} and 
{\bf SameAsBottomLayer}. The choice {\bf Constant} will set the parameter to a
constant value; {\bf PFBFile} will set the parameter to values read from a
ParFlow 2D binary file; {\bf NCFile} will set the parameter to values read
from a NetCDF file; and {\bf SameAsBottomLayer} will set the parameter to the
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the permeability
type is set to {\bf Constant}, this key will set the permeability to the
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the permeability
type is set to {\bf PFBFile} or {\bf NCFile}, this key provides ParFlow with
...
Possible values for this key are {\bf Constant, PFBFile} and {\bf NCFile}.
The choice {\bf Constant} will set the parameter to a constant value, while
{\bf PFBFile} will set the parameter to values read from a ParFlow 2D binary
file and {\bf NCFile} will set the parameter to values read from a NetCDF file.
\begin{display}\begin{verbatim}
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the specific yield
type is set to {\bf Constant}, this key will set the specific yield to the
constant value provided by the user.
\begin{display}\begin{verbatim}
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the specific yield
type is set to {\bf PFBFile} or {\bf NCFile}, this key provides ParFlow with the
...
The only possible value for this key is {\bf Constant}.
The choice {\bf Constant} will set the parameter to a constant value.
\begin{display}\begin{verbatim}
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the aquifer depth
type is set to {\bf Constant}, this key will set the aquifer depth to the
constant value provided by the user.
\begin{display}\begin{verbatim}
...
{no default}
When the {\bf DeepAquifer} boundary condition is chosen and the elevations
type is set to {\bf Constant}, this key will set the elevations to the
constant value provided by the user.
\begin{display}\begin{verbatim}
...
{This key sets the distribution of transpiration over the root zone and changes the behavior of plant water limitations.  All options use the beta-type water stress defined by \code{Solver.CLM.VegWaterStress}.  Option~0 (default) distributes transpiration by root fraction only and limits all transpiration when the top soil layer drops below the wilting point.  Option~1 distributes transpiration weighted by both root fraction and per-layer water stress, with no top-layer cutoff \cite{Ferg16}.  Option~2 adds compensatory root water uptake \cite{Li2001}: wet layers compensate for dry layers by boosting the aggregate stress factor \code{btran} up to a PFT-dependent maximum (\code{omega\_max}, set in \code{drv\_vegp.dat}).  When \code{omega\_max}=1.0 (default), this recovers option~1 behavior; values of 2--3 are recommended for deep-rooted trees.}
\begin{display}\begin{verbatim}
pfset Solver.CLM.RZWaterStress       2
\end{verbatim}\end{display}
