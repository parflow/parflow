...
an upwinded kinematic and diffusive wave overland flow routing boundary that solve equation \ref{eq:manningsnew}. The key {\bf SeepageFace} simulates a boundary that allows flow to exit but keeps the surface pressure at zero.
...
{
This key provides a lower value for the $\bar{S_{f}}$ for the diffusive wave boundary condition.
}
\begin{display}\begin{verbatim}
...
{
This key provides a lower value for the $\bar{S_{f}}$ for the kinematic wave boundary condition.
}
\begin{display}\begin{verbatim}
