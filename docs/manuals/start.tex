...
\parflow{} may also be coupled with the land surface models \code{CLM} \cite{Dai03} and \code{CoLM} \cite{Yang2026}.
The bundled land model code has been modified to be called from within \parflow{}
...
physics to support coupled operation to best utilize the integrated hydrology
 in \parflow{} \cite{MM05, KM08a}.  To couple \code{CLM} or \code{CoLM} into \parflow{} first the
 \code{--with-clm} option is needed in the \code{./configure} command as indicated in \S~\ref{Installing ParFlow}.
 Second, the \code{CLM} or \code{CoLM} needs to be called from within \parflow{}, this is done using the following
 solver key:
\begin{verbatim}
pfset Solver.LSM CLM
pfset Solver.LSM CoLM
\end{verbatim}
Note that this key is used to call \code{CLM} or \code{CoLM} from within the nonlinear solver time loop
and requires that the solver be set to RICHARDS to work.  Note also that this key defaults to
\emph{not} call a land surface model so if this line is omitted neither \code{CLM} nor \code{CoLM} will be called from within
\parflow{} even if compiled and linked.  Currently, \code{CLM} or \code{CoLM} gets some of it's information from \parflow{}
